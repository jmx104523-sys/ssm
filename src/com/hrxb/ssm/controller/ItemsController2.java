...
public class ItemsController2 {
	...
	// @Valid 表示在封装Items对象之前进行合法校验
		...
		public String addItems(Model model, @Validated(value = Validator1.class) Items items, BindingResult result) {
			// 如果有错误消息，不在执行后续操作，转发到增加页面，并进行错误提示
			if (result.hasErrors()) {
	...
	//按商品信息查询商品
	//@RequestBody 把json 对象转为 java 对象
	//@ResponseBody 把java 对象转为 json 对象
	@RequestMapping("/findItems")
	@ResponseBody
	public List<Items> findItems(@RequestBody Items items){
		PageInfo<Items> pageInfo = ItemsService.findAllItems(1, 4);
